...
// Debug endpoint to manually trigger webhook processing
app.post('/debug/trigger-webhook', async (req, res) => {
  try {
    const userId = 'YYaZizyXAkQbunFdL11LQGXl2z82';
    const firebaseAdmin = require('./services/firebaseAdmin');
    
    console.log('🧪 Manually triggering webhook-like processing...');
    
    // Simulate processing an incoming email for auto-reply
    const mockEmailData = {
      id: `mock-${Date.now()}`,
      threadId: `thread-${Date.now()}`,
      ...
      body: 'This is a test email to trigger auto-reply processing.',
      date: new Date().toISOString()
    };
    ...
    if (settingsSnapshot.empty) {
      return res.json({
        success: false,
        message: 'No active auto-reply settings found',
        suggestion: 'Configure auto-reply settings first'
      });
    }
    
    const settings = settingsSnapshot.docs[0].data();
    console.log('📋 Found active auto-reply settings:', settings);
    
    // Add to auto-reply queue (simulating what gmailWatchService should do)
    const queueItem = {
      status: 'pending',
      userId: userId,
    ...
    console.log('✅ Added email to auto-reply queue:', queueRef.id);
